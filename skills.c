...
void driveFor(float goal) {//drives for certain inches
	...
	while (abs(goal * circum - encoderAvg*encoderScale) > thresh) {
		fwds(limitDownTo(15, dP * ((goal*circum - encoderAvg*encoderScale - 0.3*mainVelocity))), initDir);
	}
	fwds(0, initDir);
	return;
}
...
void clawControl(int open, int close = 0){
	if (open) {
		motor[ClawMotor] = 80;
		delay(50);
	}
...
task MechControlTask() {
	...
	for (;;) {//while true
		...
		limitMechControl(&lock.m, U8, D8, U8_2, D8_2, lock.min, lock.max, true);
...
task usercontrol() {//initializes everything
	...
	for (;;) {
		...
		if(D7) driveFor(-20);
		LiftLift(&fourBar, D5, U5, D5_2, U5_2, 5000, false);
		//DownUntil(&fourBar, fourBar.min, 80);
		//mechMove(&fourBar.m, 0);
		//delay(500);
		//UpUntil(&fourBar, fourBar.max, 80);
		//mechMove(&fourBar.m, 0);
		delay(15);//~60hz
	}
