...
#pragma config(Motor,  port5,           FourBarMotor,       tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port10,          LockMotor,     tmotorVex393_HBridge, openLoop)
...
void initLift(struct liftMech* lift, char sensor, int motre, int min, int max, int delayamnt = 20) {
	lift->min = min;
	lift->min = max;
...
void initializeOpControl(const bool driver) {
	...
	initLift( &lock,								 LockPot,			LockMotor,		1200	,	2000);//(min) && (max)
	initLift( &fourBar,								FourBarPot,	FourBarMotor,			1500,	3900)
