...
enum mechType { DUALSENSOR, NORMAL, DRIVE, BINARY};//what kinds of lift we have
...
* '----------------'  '----------------'  '----------------'  '----------------' *
\********************************************************************************/
void initMechType(const struct liftMech* lift, enum mechType type, char sensor, char sensor2, int m1, int m2, int max, int min, int delayAmnt = 20) {
	lift->m.type = type;
	lift->m.sensor[0] = sensor;
	lift->m.sensor[1] = sensor2;//null second sensor (FOR ALL LIFTS OTHER THAN MAINLIFT)
	lift->m.motors[0] = m1;
	lift->m.motors[1] = m2;
	lift->max = max;
	lift->min = min;
	...
	lift->m.velocity = 0.0;
	lift->past = 0;
	lift->goal = SensorValue[lift->m.sensor[0]];
}
...
void initsideMech(const struct sideMech* side, char sensor, int m1, int m2){
	...
	side->m.motors[0] = m1;
	side->m.motors[1] = m2;
	side->m.velocity = 0;//initially not moving
	side->m.stalling = false;//initially not stalling (i hope)
	SensorValue[side->m.sensor] = 0;//initially resets encoders
	side->m.past = 0;
}
void scaleGyros(){
...
void initializeOpControl(const bool driver) {
	...
	initMechType(	&mainLift,	DUALSENSOR,	LeftLiftPot,	RightLiftPot,	LiftRight,	LiftLeft, 	2150,	 620	  );
	initMechType(	&FourBar,	NORMAL,	 	FourBarPot,		0,				R4Bar,		L4Bar,		2550,	 1020,	10);
	initMechType(	&MoGo,		NORMAL,		MoGoPot,		0,				RMogo,		LMogo,		4000,	 2000,	  );

	//-PID------&reference------sensor----------------thresh----kP-------kI-----kD------reversed----running(opt)
	initPID(	&mainLift.PID,	mainLift.m.sensor[0],	  30,	    0.45,	 0.0,	0.05, 	true, 		true);
	initPID(	&FourBar.PID, 	FourBar.m.sensor[0], 	  10, 	    0.1, 	 0.0,   0.01,   true, 		true);
	initPID(	&MoGo.PID, 		MoGo.m.sensor[0], 	  30, 	    0.15,    0.0,   0.0,    true, 		true);
...
void liftMove(const struct liftMech* lift, const float speed) {
	...
	if(lift->m.sensor[1] != 0 && lift->m.type == DUALSENSOR){//HAS to be mainLift because has initialized 2nd sensor
		const float scalar = 0.05;//scalar for potentiometer difference
		float powSkew = limitUpTo(20, scalar*(PolyReg(SensorValue[mainLift.m.sensor[0]]) - SensorValue[mainLift.m.sensor[1]]));
		motor[lift->m.motors[0]] = power - powSkew;//one side goes faster/slower to compensate
		motor[lift->m.motors[1]] = -(power - powSkew);//one side goes faster/slower to compensate
	}
	else {
		motor[lift->m.motors[0]] = power;//up is fast
		motor[lift->m.motors[1]] = -power;//up is fast
	}
}
...
void manualLiftControl(const struct liftMech* lift, int bUp, int bDown, int bUp2, int bDown2, bool reversed, int maxSpeed) {
	...
	else if ((SensorValue[lift->m.sensor] >= lift->max && (upButton))
		|| (SensorValue[lift->m.sensor] <= lift->min && (downButton)))//pressing buttons but surpassed limits
...
void LiftLift(const struct liftMech* lift, int bUp, int bDown, int bUp2, int bDown2, float velLimit) {
	...
	else if(lift->m.type != BINARY){//BINARY IS ONLY TOP/BOTTOM
		if (abs(SensorValue[lift->m.sensor] - lift->goal) < 200 || abs(lift->m.velocity) < velLimit) {
...
void UpUntilStack(const struct liftMech* lift, int goal, int speed) {//uses sonar for bringing lift up
	lift->PID.isRunning = false;
	int currentPos;
	while (SensorValue[lift->m.sensor] < goal || (SensorValue[sonar] <= 13 && SensorValue[sonar] >= 0)) {//brings lift up to goal (ACCOUNTS FOR SONAR)
		liftMove(lift, speed);
		currentPos = SensorValue[lift->m.sensor];
		if(stopAutoStack) return;
	}
	if(SensorValue[sonar] <= 15 && SensorValue[sonar] >= 0){
		while(SensorValue[lift->m.sensor] < currentPos + 150){
...
task LiftControlTask() {
	for (;;) {//while true
		if(!autonRunning){
			...
			if(SensorValue[MoGo.m.sensor[0]] > MoGo.max + 350){
				liftMove(&MoGo, 80);//keeps mogo down
			}
		}
		else {
...
void driveLR(const int powerR, const int powerL) {
	if(autonRunning){
		if(!Right.m.stalling) mechMove(&Right.m, powerR);
		else {
			mechMove(&Right.m, 0);
			delay(750);//stall waiter
		}
		if(!Left.m.stalling) mechMove(&Left.m, powerL);
		else {
			mechMove(&Left.m, 0);
			delay(750);//stall waiter
		}
		...
		mechMove(&Right.m, powerR);
		mechMove(&Left.m, powerL);
	}
}
void driveCtrlr() {
	//scale for joystick
...
* '----------------'  '----------------'  '----------------'  '----------------' *
\********************************************************************************/
float calcVel(const struct mechanism* mech, const float dist, const float delayAmount) {
	float velocity = ((SensorValue(mech->sensor) - mech->past) / dist) / ((float)(delayAmount / 1000));//1000ms in 1s
...
task MeasureSpeed() {
	...
	for (;;) {
		//base velocity
		Right.m.velocity = calcVel(&Right.m, circum, delayAmount);
		Left.m.velocity = calcVel(&Left.m, circum, delayAmount);
		velocity = avg(Right.m.velocity, Left.m.velocity);//overall velocity (avdg between the two)
		rotVelocity = calcRotVel();//calculates rotational velocity
		//lift velocities
		mainLift.m.velocity = calcVel(&mainLift.m, dist, delayAmount);
		FourBar.m.velocity = calcVel(&FourBar.m, dist, delayAmount);
		MoGo.m.velocity = calcVel(&MoGo.m, dist, delayAmount);
		//does the waitings
		delay(delayAmount);
	}
...
void displayBatteryLevels(){
	...
	sprintf(mainBattery, "%1.2f%c", BackupBatteryLevel/1000.0,'V'); //Build the value to be displayed
...
task sensorsUpdate() {
	...
	for (;;) {
		...
		encoderAvg = avg(SensorValue[Right.m.sensor], SensorValue[Left.m.sensor]);
...
task antiStall(){
	for(;;){
		checkStalling(&Right.m);
		checkStalling(&Left.m);
		delay(50);
	}
}
/********************************************************************************\
...
void threeConeAuton(const bool left){
	...
	driveFor(2);
	mainLift.PID.isRunning = true;
	mainLift.goal = SensorValue[mainLift.m.sensor[0]] + 200;
	delay(250);
	DownUntil(&FourBar, FourBar.min, 127);//ensures 4bar is down
	UpUntil(&mainLift, SensorValue[mainLift.m.sensor[0]] + 200, 127);
